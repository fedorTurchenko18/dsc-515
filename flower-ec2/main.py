...
if __name__=='__main__':
    ...
    local_log_path = f'{local_log_path}/server_log.txt'
    out_cond = False
    while out_cond == False:
        fl_log_download_response = s3_manager.download_from_s3_bucket(local_file_path=local_log_path, object_key='tensorflow/FedAvg/server_log.txt')
        try:
            if 'error' in fl_log_download_response:
                out_cond = False
        except TypeError:
            out_cond = True