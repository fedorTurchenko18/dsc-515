        ...
        def preprocess_images(examples):
            ...
            for image in examples['image']:
                ...
                if len(np_image.shape) == 2:  # Grayscale image
                    ...
                    np_image = np.concatenate([np_image, np_image, np_image], axis=-1)
                elif np_image.shape[2] == 1:  # Single channel image
                    np_image = np.concatenate([np_image, np.zeros_like(np_image), np.zeros_like(np_image)], axis=-1)
                processed_images.append(np_image / 255.0)
